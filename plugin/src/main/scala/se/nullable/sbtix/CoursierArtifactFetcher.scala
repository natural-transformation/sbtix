...
class CoursierArtifactFetcher(logger: Logger, resolvers: Set[Resolver], credentials: Map[String, Credentials]) {
  ...
  private def getAllDependencies(modules: Set[Dependency]): (Set[(Dependency, Artifact)], ResolutionErrors) = {
    ...
    def go(modules: Set[Dependency]): (Set[(Dependency, Artifact)], ResolutionErrors) = {
      ...
      val resolvedMissingDependencies = missingDependencies.map(dep => go(Set(dep)))
      val artifacts = resolution.dependencyArtifacts(true).toSet
      ...
      val errors = resolvedMissingDependencies.foldRight(ResolutionErrors(resolution.errors))((resolved, acc) => acc + resolved._2)
      (artifacts, errors)
    }
  ...
  private def diffDependencyTrees(raw: Fix[F], reconciled: Fix[F]): Fix[G] = {
    val coalgebra: Coalgebra[G, Either[Fix[F], (Fix[F], Fix[F])]] = {
      case Right((raw, reconciled)) =>
        ...
        val recMap = reconciled.unFix.children.map { t => t.unFix.value -> t.unFix.children }.toMap
