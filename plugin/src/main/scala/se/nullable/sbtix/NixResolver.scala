...
case class NixResolver(private val name: String,
                       rootUrl: String,
  ...
  def filterArtifacts(
      logger: Logger,
      modules: Set[GenericModule]): Option[(NixRepo, Set[NixArtifact])] = {
  ...
  def filterMetaArtifacts(
      logger: Logger,
      metaArtifacts: Set[MetaArtifact]): Option[(NixRepo, Set[NixArtifact])] = {
