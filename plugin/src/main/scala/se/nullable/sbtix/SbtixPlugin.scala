...
object SbtixPlugin extends AutoPlugin {
  ...
  private def resolveSbtixSourceBlock(pluginVersion: String): SourceBlock = {
    ...
    def nixPathExpr(path: String): String = {
      // Prefer emitting a Nix path literal when possible. This is both simpler
      // and avoids accidentally creating stringly-typed path values.
    ...
    // Prefer a store-backed source path when available. This keeps Nix builds
    // fully offline (no GitHub fetch) while still producing deterministic
    // generated files.
    maybeSourcePath match {
      ...
      case None =>
        // If rev + narHash are provided, embed a pinned sbtix source block.
        // narHash feeds fetchTree; sha256 remains for the fetchgit fallback.
        // Generated files are regenerated by sbtix-gen-all2; do not hand-edit.
